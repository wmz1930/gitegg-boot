...
@Slf4j
@RestController
@RequestMapping("/extension/wx/pay/ent/{appId}")
@RequiredArgsConstructor(onConstructor_ = @Autowired)
@Api(value = "WxPayController|微信企业付款配置前端控制器", tags = {"微信企业付款配置"})
public class EntPayController {
    ...
    public EntPayQueryResult queryEntPay(@PathVariable String appId, @PathVariable String partnerTradeNo) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public EntPayBankQueryResult queryPayBank(@PathVariable String appId, @PathVariable String partnerTradeNo) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
