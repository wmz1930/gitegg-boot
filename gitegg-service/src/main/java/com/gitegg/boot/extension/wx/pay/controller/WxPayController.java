...
@Slf4j
@RestController
@RequestMapping("/extension/wx/pay/{appId}")
@RequiredArgsConstructor(onConstructor_ = @Autowired)
@Api(value = "WxPayController|微信支付配置前端控制器", tags = {"微信支付配置"})
public class WxPayController {
    ...
    public WxPayOrderQueryResult queryOrder(@PathVariable String appId,
        ...
        if (!payService.switchover(appId)) {
    ...
    public WxPayOrderQueryResult queryOrder(@PathVariable String appId, @RequestBody WxPayOrderQueryRequest wxPayOrderQueryRequest) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayOrderCloseResult closeOrder(@PathVariable String appId, @PathVariable String outTradeNo) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayOrderCloseResult closeOrder(@PathVariable String appId, @RequestBody WxPayOrderCloseRequest wxPayOrderCloseRequest) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayUnifiedOrderResult unifiedOrder(@PathVariable String appId, @RequestBody WxPayUnifiedOrderRequest request) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    @PostMapping("/refund")
    public WxPayRefundResult refund(@PathVariable String appId, @RequestBody WxPayRefundRequest request) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayRefundQueryResult refundQuery(@PathVariable String appId,
                                              @RequestBody WxPayRefundQueryRequest wxPayRefundQueryRequest) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public String parseOrderNotifyResult(@PathVariable String appId,
    ...
    public String parseRefundNotifyResult(@PathVariable String appId,
                                          @RequestBody String xmlData) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPaySendRedpackResult sendRedpack(@PathVariable String appId, @RequestBody WxPaySendRedpackRequest request) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayRedpackQueryResult queryRedpack(@PathVariable String appId, @PathVariable String mchBillNo) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
     ...
     * @return 生成的二维码的字节数组
     */
    public byte[] createScanPayQrcodeMode1(@PathVariable String appId, String productId, File logoFile, Integer sideLength) {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
     ...
     * @return 生成的二维码的字节数组
     */
    public byte[] createScanPayQrcodeMode2(@PathVariable String appId, String codeUrl, File logoFile, Integer sideLength) {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayBillResult downloadBill(@PathVariable String appId,
    ...
    public WxPayBillResult downloadBill(@PathVariable String appId,
                                        WxPayDownloadBillRequest wxPayDownloadBillRequest) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayMicropayResult micropay(@PathVariable String appId,
                                        @RequestBody WxPayMicropayRequest request) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayCouponSendResult sendCoupon(@PathVariable String appId, @RequestBody WxPayCouponSendRequest request) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayCouponStockQueryResult queryCouponStock(@PathVariable String appId, @RequestBody WxPayCouponStockQueryRequest request) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public WxPayCouponInfoQueryResult queryCouponInfo(@PathVariable String appId, @RequestBody WxPayCouponInfoQueryRequest request) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
    ...
    public String queryComment(@PathVariable String appId, Date beginDate, Date endDate, Integer offset, Integer limit) throws WxPayException {
        if (!payService.switchover(appId)) {
            throw new IllegalArgumentException(String.format("未找到对应appId=[%s]的配置，请核实！", appId));
        }
