...
public class MultiWebSecurityConfig {
    ...
    /**
     * 内存用户Basic校验，主要应用于Swagger等权限校验
     */
    @ConditionalOnProperty( value = {"knife4j.basic.enable"}, havingValue = "true")
    @Configuration
    @Order(2)
    public class SwaggerConfigurationAdapter extends WebSecurityConfigurerAdapter {

        public SwaggerConfigurationAdapter() {
            super();
        }
        ...
        @Override
        protected void configure(HttpSecurity http) throws Exception {
            http.antMatcher("/**/api-docs").authorizeRequests().anyRequest().hasAnyRole(securityProperties.getUser().getRoles().toArray(new String[0]))
                    .and().httpBasic().and().csrf().disable();
        }
    }

    /**
     * 这个Order一定要设置为3，在ResourceServer之后，JDBC用户权限校验
     */
    @Configuration
    @Order(5)
