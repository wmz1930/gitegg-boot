...
public class SocialTokenGranter extends AbstractTokenGranter {
    ...
    private IJustAuthService justAuthService;
    ...
    public SocialTokenGranter(AuthenticationManager authenticationManager,
                              ...
                              OAuth2RequestFactory requestFactory, RedisTemplate redisTemplate, IJustAuthService justAuthService,
    ...
    protected OAuth2Authentication getOAuth2Authentication(ClientDetails client, TokenRequest tokenRequest) {
        ...
        String userId;
        try {
            Result<Object> socialResult = justAuthService.userBindQuery(Long.parseLong(socialId));
            if (null == socialResult || StringUtils.isEmpty(socialResult.getData())) {
                throw new InvalidGrantException("操作失败，请返回登录页重新操作");
            }
            userId = String.valueOf(socialResult.getData());
        }
        catch (Exception e)
        {
            throw new InvalidGrantException("操作失败，请返回登录页重新操作");
